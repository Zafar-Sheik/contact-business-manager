...
interface QuoteItemManagerProps {
  ...
  setItems: React.Dispatch<React.SetStateAction<QuoteItemInsert[]>>;
  availableStock: StockItemForInvoice[];
}

const QuoteItemManager: React.FC<QuoteItemManagerProps> = ({
  items,
  setItems,
  availableStock,
}) => {
  ...
  const calculateLineTotal = (item: QuoteItemInsert) => {
    ...
    <Card className="p-4">
              ...
              >
                <div className="flex-1 min-w-0 pr-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Qty: {item.quantity} @ {formatCurrency(item.unit_price)} (VAT: {item.vat_rate}%)
                  </p>
                </div>
      ...
      <div className="space-y-1 text-sm">
        ...
        <div className="flex justify-between text-lg font-bold border-t pt-2 mt-2">
          <span>TOTAL QUOTE (Incl. VAT):</span>
