...
export type InvoiceItemInsert = Omit<InvoiceItem, 'id' | 'invoice_id' | 'created_at' | 'line_total'> & {
  line_total?: number;
};
...
export type InvoiceInsert = Omit<Invoice, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'subtotal' | 'vat_amount' | 'total_amount' | 'status' | 'is_vat_invoice'> & {
  status?: 'Draft' | 'Sent' | 'Paid' | 'Cancelled';
